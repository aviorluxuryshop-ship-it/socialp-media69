...
export function ProductScene({ quality, reducedMotion }: ProductSceneProps) {
  ...
  const colors = useMemo(
    () => ({
      cold: new THREE.Color(SCENE.background.cold),
      warm: new THREE.Color(SCENE.background.warm),
      live: new THREE.Color(SCENE.background.cold),
    }),
    [],
  )
  ...
  return (
    <>
      ...
      <BackdropGlow amount={glow} />

      <ProductModel ref={model} segments={quality === 'high' ? 128 : 72} labelScale={quality === 'high' ? 1.5 : 1} onReady={onReady} />
      ...
      <AromaAccents amount={aroma} particles={quality === 'high' ? 26 : 12} />
