    ...
    <section id={id} className="relative" style={{ height: `${sceneHeight(id)}vh` }}>
      <div className="sticky top-0 h-[100svh] w-full">
        ...
        >
          <div
            className={clsx(
              layout === 'side' && 'max-w-md md:max-w-[26rem]',
              layout === 'bottom' && 'max-w-xl',
              // the macro shots fill the whole frame behind it, so the corner label needs its own contrast, not just the scene's darkness
              layout === 'corner' && 'max-w-[19rem] rounded-2xl bg-void/55 px-4 py-3 backdrop-blur-sm md:bg-transparent md:px-0 md:py-0 md:backdrop-blur-none',
            )}
          >
            {children}
          </div>
