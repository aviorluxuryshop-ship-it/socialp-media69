...
export type Frame = {
  ...
  rimLight: number
  envIntensity: number
  /** 0..1 — soft backdrop glow behind the can, tinted to the active flavor. */
  glow: number
  /** 0..1 — shifts the backdrop from neutral black toward a warm, flavor-tinted black. */
  warmth: number
  /** 0..1 — fruit slices, leaves and particles of the flavors scene. */
  aroma: number
...
export const TIMELINE: Frame[] = [
  ...
  // FLAVORS — pull back to a 3/4 angle, fruit slices and leaves drift in beside the can while the picker is used
  key('aromalar', 0, { pos: [0.35, 0.1, 3.1], lookAt: [0, 0, 0], fov: 30, rotY: -TURN - 0.9, keyLight: 1.2, glow: 0.7, warmth: 1, aroma: 1, ...SIDE_COPY }),
  key('aromalar', 1, { pos: [0.4, 0.12, 3.0], lookAt: [0, 0.01, 0], fov: 30, rotY: -TURN - 1.1, keyLight: 1.2, glow: 0.7, warmth: 1, aroma: 1, ...SIDE_COPY }),

  // BRAND — editorial and calm: the can steps back (further on phones, where the producer block runs long) to give the copy room
  key('marka', 0, { pos: [0.3, 0.06, 3.35], lookAt: [0, 0, 0], fov: 31, rotY: -TURN - 1.1, keyLight: 1.1, rimLight: 1, glow: 0.45, warmth: 0.7, aroma: 0.15, ...SIDE_COPY, mobileY: 0.48, mobileZoom: 1.95 }),
  key('marka', 1, { pos: [0.24, 0.02, 3.5], lookAt: [0, -0.1, 0], fov: 32, rotY: -TURN - 0.55, keyLight: 1.08, rimLight: 1.05, glow: 0.55, warmth: 0.35, aroma: 0, ...SIDE_COPY, mobileY: 0.48, mobileZoom: 1.95 }),

  // FINAL HERO — centred, upright, strong rim light, camera eases back and holds
